...
from . import schemas
from .db import Post, get_async_session, create_db_and_tables, User
...
app.include_router(fastapi_users.get_verify_router(schemas.UserRead), prefix="/auth", tags=["auth"])
app.include_router(fastapi_users.get_users_router(schemas.UserRead, schemas.UserUpdate), prefix="/users", tags=["users"])
...
async def delete_post(post_id: str,session: AsyncSession = Depends(get_async_session), user : User = Depends(current_active_user)):
    try:
        ...
        if not post:
            raise HTTPException(status_code=302, detail="No post with this ID")
